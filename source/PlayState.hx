...
class PlayState extends MusicBeatState
{
	...
	public var finalHealth:Float = 1;
	...
	override public function create()
	{
		...
		var keTimeBar:Bool = (ClientPrefs.timeBarStyle == 'Kade Engine');
		...
		timeBarBG.xAdd = -4;
		timeBarBG.yAdd = -4;
		if (keTimeBar) // bedrock engine my beloved
		...
		if (ClientPrefs.timeBarType == 'Song Name' && keTimeBar)
		{
			timeTxt.size = 18;
			timeTxt.y += 5;
		}
	...
	public function updateScore()
	{
		...
		{
			case 'Random Engine':
				scoreTxt.text = lol + ' // Misses: ' + songMisses + ' // Acc: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ' // Rank: '
					+ ratingName + ' (' + ratingFC + ')'; // peeps wanted no integer rating
			case 'Psych Engine':
				scoreTxt.text = lol + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)'
	...
	{ // STOLE FROM ANDROMEDA
		...
		{ // to avoid epilepsia or smth idk
	...
	if (!foundFile)
	{
			...
			/*if (FlxG.keys.justPressed.NINE)
		{
			...
			if (ClientPrefs.smoothHealth)
				finalHealth = FlxMath.lerp(finalHealth, health, CoolUtil.boundTo(elapsed * 7, 0, 1));
			else
				finalHealth = health;
		...
		{
			if (((skipHealthCheck && instakillOnMiss) || finalHealth <= 0) && !practiceMode && !isDead)
			{
